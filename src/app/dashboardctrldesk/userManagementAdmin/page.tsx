...
const fetchUsers = async (page: number, search?: string) => {
  ...
  const { data, error } = await fetchWithCookie(
    `${apiRoutesconsole.GET_USER_CTRLDSK_ADMIN}?${queryParams}`,
    "GET"
  );
...
const createEditUrl = (user: User) => {
  ...
  return `/dashboardctrldesk/userManagementAdmin/createUserAdmin?${params.toString()}`;
};
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex items-center justify-between">
          <h1 className="text-2xl font-bold text-[#0C3C60]">User Management Admin</h1>
            ...
            onClick={() => {
              window.location.href = "/dashboardctrldesk/userManagementAdmin/createUserAdmin";
            }}
            >
            + Create User
            </Button>

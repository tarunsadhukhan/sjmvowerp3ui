...
export default function ManMachineFinalProcessDialog({ open, onClose, onSuccess }: Props) {
	...
	const handleSubmit = async () => {
		...
		setBusy(true);
		try {
			const url = `${apiRoutesPortalMasters.MAN_MACHINE_FINAL_PROCESS}?co_id=${co_id}`;
			const { data, error: err } = await fetchWithCookie<{ message: string }>(url, "POST", {
				tran_date: tranDate,
				branch_id,
			});
	...
	return (
		<Dialog open={open} onClose={busy ? undefined : onClose} maxWidth="xs" fullWidth>
			<DialogTitle>Daily Man-Machine — Final Process</DialogTitle>
			<DialogContent>

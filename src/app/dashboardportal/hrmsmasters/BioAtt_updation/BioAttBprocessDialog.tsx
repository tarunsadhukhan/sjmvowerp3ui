...
export default function BioAttBprocessDialog({ open, onClose, onSuccess }: Props) {
	...
	const [result, setResult] = useState<BprocessResponse | null>(null);
	...
	const handleProcess = useCallback(async () => {
		...
		try {
			const url = `${apiRoutesPortalMasters.BIO_ATT_BPROCESS}?co_id=${encodeURIComponent(co_id)}`;
			const resp = await axios.post<BprocessResponse>(
				url,
			...
			setResult(resp.data);
			onSuccess?.(
				`Bprocess: resolved ${resp.data.resolve.resolved} emp(s), ` +
			...
			setError(
				ax.response?.data?.detail ?? ax.message ?? "Bprocess failed",
			);
		} finally {
			setBusy(false);
		}
	...
	return (
		<Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
			<DialogTitle>Bprocess</DialogTitle>
				...
				<Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
					...
					{result && (
						<Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
							...
							<Typography variant="body2">Total inserted: {result.process.total_inserted}</Typography>
							{result.is_off_day && (

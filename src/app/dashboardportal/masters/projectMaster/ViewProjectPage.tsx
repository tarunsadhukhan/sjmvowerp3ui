...
export default function ViewProjectPage({ open = false, onClose, project_id }: Props) {
  ...
  useEffect(()=>{ if (!open || !project_id) return; (async ()=>{
    ...
    try {
      ...
      // prefer an object under project or project_master
      const rec = candidate?.project || candidate?.project_master || candidate;
      setRawResponse(candidate);
      setRecord(rec);
    } catch(err:any){ console.warn('ViewProjectPage failed to load', err?.message || err); setRawResponse(null); setRecord(null); }
    finally{ setLoading(false); }
  })(); }, [open, project_id]);

  return (
    <Dialog open={open} onClose={() => onClose && onClose()} maxWidth="sm" fullWidth>
      {record ? <CreateProjectPage open={true} onClose={onClose} readOnly={true} initialValues={record} /> : <div style={{ padding: 16 }}>{loading ? 'Loading...' : (rawResponse ? <pre>{JSON.stringify(rawResponse, null, 2)}</pre> : 'No details')}</div>}
    </Dialog>
  );
}

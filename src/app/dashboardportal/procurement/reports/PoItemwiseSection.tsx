...
import { usePoReport } from "./hooks/usePoReport";
import PoItemwiseReport from "./_components/PoItemwiseReport";
...
interface PoItemwiseSectionProps {
  selectedCompany: {
    co_id?: number;
...
export default function PoItemwiseSection({ selectedCompany }: PoItemwiseSectionProps) {
  const { rows, total, loading, error, loadReport } = usePoReport();
  ...
  const handleExport = useCallback(async () => {
    ...
    setExporting(true);
    try {
      const result = await fetchPoItemwiseReport({
        page: 1,
        limit: 10000,
        co_id: coId,
        branch_id: selectedBranch ? String(selectedBranch.branch_id) : undefined,
      ...
      exportToCSV(result.data, "po_itemwise_report.csv");
    } catch {
      // Export failed silently — data grid already shows the data
    } finally {
      setExporting(false);
    }
  }, [coId, selectedBranch, dateFrom, dateTo, poType, outstandingFilter, searchValue]);

"use client";

import React from "react";
import { Typography, Chip } from "@mui/material";
import { DataGrid } from "@mui/x-data-grid";
import type { GridColDef, GridPaginationModel } from "@mui/x-data-grid";
import type { PoReportRow } from "../types/reportTypes";

const formatDate = (value?: string) => {
  if (!value) return "-";
  const trimmed = value.trim();
  const ymdMatch = trimmed.match(/^([0-9]{4})-([0-9]{2})-([0-9]{2})$/);
  if (ymdMatch) {
    const [, year, month, day] = ymdMatch;
    const date = new Date(Number(year), Number(month) - 1, Number(day));
    if (!Number.isNaN(date.getTime())) {
      return new Intl.DateTimeFormat("en-GB", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      }).format(date);
    }
  }
  return trimmed;
};

const fmtQty = (value: number | null | undefined) =>
  value != null ? Number(value).toLocaleString("en-IN", { maximumFractionDigits: 2 }) : "-";

const fmtRate = (value: number | null | undefined) =>
  value != null ? Number(value).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "-";

const statusColor = (status: string) => {
  const s = status?.toLowerCase();
  if (s === "approved") return "success" as const;
  if (s === "rejected") return "error" as const;
  if (s === "closed") return "info" as const;
  return "default" as const;
};

const columns: GridColDef[] = [
  {
    field: "po_no",
    headerName: "PO No.",
    minWidth: 180,
    flex: 1,
    renderCell: (params) => (
      <Typography component="span" variant="body2" color="primary" sx={{ fontWeight: 600 }}>
        {params.value || "-"}
      </Typography>
    ),
  },
  {
    field: "po_date",
    headerName: "PO Date",
    minWidth: 120,
    renderCell: (params) => formatDate(params.value),
  },
  { field: "branch_name", headerName: "Branch", minWidth: 140, flex: 1 },
  { field: "supplier_name", headerName: "Supplier", minWidth: 180, flex: 1.5 },
  { field: "item_name", headerName: "Item", minWidth: 200, flex: 1.5 },
  { field: "item_grp_name", headerName: "Item Group", minWidth: 150, flex: 1 },
  { field: "uom_name", headerName: "UOM", minWidth: 80 },
  {
    field: "po_qty",
    headerName: "PO Qty",
    minWidth: 100,
    type: "number",
    renderCell: (params) => fmtQty(params.value),
  },
  {
    field: "rate",
    headerName: "Rate",
    minWidth: 100,
    type: "number",
    renderCell: (params) => fmtRate(params.value),
  },
  {
    field: "inward_consumed_qty",
    headerName: "Inward Consumed",
    minWidth: 140,
    type: "number",
    renderCell: (params) => fmtQty(params.value),
  },
  {
    field: "outstanding_qty",
    headerName: "Outstanding",
    minWidth: 120,
    type: "number",
    renderCell: (params) => (
      <Typography
        component="span"
        variant="body2"
        sx={{ fontWeight: 600, color: Number(params.value) > 0 ? "#d32f2f" : "inherit" }}
      >
        {fmtQty(params.value)}
      </Typography>
    ),
  },
  { field: "po_type", headerName: "PO Type", minWidth: 110 },
  { field: "expense_type_name", headerName: "Expense Type", minWidth: 130 },
  {
    field: "status_name",
    headerName: "Status",
    minWidth: 120,
    renderCell: (params) => (
      <Chip
        size="small"
        color={statusColor(params.value ?? "")}
        label={params.value || "Pending"}
      />
    ),
  },
];

interface PoItemwiseReportProps {
  rows: PoReportRow[];
  rowCount: number;
  loading: boolean;
  paginationModel: GridPaginationModel;
  onPaginationModelChange: (model: GridPaginationModel) => void;
}

export default function PoItemwiseReport({
  rows,
  rowCount,
  loading,
  paginationModel,
  onPaginationModelChange,
}: PoItemwiseReportProps) {
  return (
    <DataGrid
      rows={rows}
      columns={columns}
      getRowId={(row) => row.po_dtl_id}
      rowCount={rowCount}
      loading={loading}
      paginationMode="server"
      paginationModel={paginationModel}
      onPaginationModelChange={onPaginationModelChange}
      pageSizeOptions={[10, 25, 50, 100]}
      disableRowSelectionOnClick
      autoHeight
      sx={{
        border: 1,
        borderColor: "divider",
        "& .MuiDataGrid-columnHeaders": {
          backgroundColor: "#f5f5f5",
          fontWeight: 700,
        },
      }}
    />
  );
}

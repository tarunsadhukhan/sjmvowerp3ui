...
export default function SalesQuotationIndexPage() {
		...
		},
		{
			...
			renderCell: (params: GridRenderCellParams<QuotationRow, string>) => (
				<Typography component="span" variant="body2">
					{params.value || formatDate(typeof params.row.quotation_date_raw === "string" ? params.row.quotation_date_raw : "") || "-"}
				</Typography>
			),
		},
		{
	...
	const fetchQuotations = React.useCallback(async () => {
		...
		try {
			...
			const url = `${apiRoutesPortalMasters.QUOTATION_TABLE}?${query.toString()}`;
			...
			const mappedRows: QuotationRow[] = rawRows.map((row: any) => {
				const rawDate = row.quotation_date ?? row.created_at ?? "";
				const normalizedRaw = typeof rawDate === "string" ? rawDate : rawDate ? String(rawDate) : "";
				return {
					id: row.sales_quotation_id ?? row.id ?? `${row.quotation_no ?? "sq"}-${Math.random().toString(36).slice(2, 8)}`,
